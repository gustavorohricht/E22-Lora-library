...
//Code to set the E220 transceiver parameters
...
//REGISTERS
//The only difference from E22 and E220 is that the E220 does not have the NETID register
#define ADDH_REG 0x00
#define ADDL_REG 0x01
#define REG0 0x02
#define REG1 0x03
#define CHANNEL_REG 0x04
#define REG3 0x05
#define CRYPT_H_REG 0x06
#define CRYPT_L_REG 0x07 
...
uint8_t REG0_val = 0b01100010; //Default value of the REG0 register
...
void setup() {
  ...
  // Setting to Deep Sleep Config Mode, datasheet page 11
 ...
 /*
    ...
    Preferably plug the ground wire first, the E220 is an ESD device, so it could possibly be damaged by eletrostatic discharges
 */
  ...
  However, the CHANNEL register should be the same for both modules in order to be able to communicate
  The channels range from 0-80. set the channel as your transmission requirements

  */
  //Setting REG0 register
  //WriteToRegister(REG0, REG0_val);
...
void ReadRegister(uint8_t reg, uint8_t *val) {
  ...
  /*
    ...
    If you want to read more than 1 byte, you can change 0x01 to the number of bytes you want to read, just change the size of the buffer array
  */
...
void PrintRegisterValue(uint8_t reg) {
  ...
  /*
    ...
    Adresses are printed in hexadecimal format as it is the usual way to display them
